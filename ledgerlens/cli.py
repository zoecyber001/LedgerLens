...
def _run_financial_audit_pipeline(
    ...
    from ledgerlens.ingestion.migration_replayer import MigrationChainReplayer
    ...
    click.secho("\n  [Phase 1] Schema Ingestion, AST Parsing & Migration Replay...", fg="white")
    discovered_files = list(discover_schema_files(target.root_path))
    parsed_files = []
    for f in discovered_files:
        p_file = parse_sql_ast(f)
        if not p_file.tables:
            p_file = parse_schema_file(f)
        parsed_files.append(p_file)

    # Reconstitute chronological migration chain into unified final state if multiple migration files exist
    if len(parsed_files) > 1:
        reconstituted = MigrationChainReplayer.reconstitute_migration_chain(parsed_files)
        parsed_files.append(reconstituted)
    ...
    click.secho(f"  [+] Discovered {len(discovered_files)} schema files ({total_tables} tables, {total_ledger_tables} financial ledger tables)", fg="green")
